...
# ── SENARYO PARAMETRELERİNİN AÇIKLAMALARI ────────────────────────────────────
# • TURN_AWARE_AKTIF: Rota arama esnasında açısal dönüş maliyetlerini hesaba katar (keskin manevraları ve gereksiz U dönüşlerini engeller).
# • BLOK_SERT_AKTIF: Engellerin olduğu yol segmentlerini grafikten tamamen siler. False ise sadece yüksek ceza puanı verilir.
# • IKI_YONLU_DURAK_AKTIF: Durak ceplerine her iki yönden de girilebilmesini sağlar (cep içini çift yönlü yapar).
# • SLALOM_ENJEKSIYON_AKTIF: Duba algılandığında duba etrafından kaçmak/sollamak için dinamik şerit değiştirme yolları oluşturur.
# • HESAP_KILIDI_AKTIF: Araç sollama manevrasına başladığında rotanın değişmesini kilitler; rotada titreme/kararsızlığı (ping-pong) önler.
# • B_PLAN_YAW_ROTA_AKTIF: Grafın tamamen tıkalı olması gibi acil durumlarda grafik dışı yaw-tabanlı sentetik slalom yolları üretir.
# • SADECE_ENGELDE_YENIDEN_PLANLA: True ise periyodik rota planlamayı kapatır; yalnız yeni duba tespiti veya duba kaybında planlama tetikler.
# • SNAP_YAW_AGIRLIK: Haritada başlangıç düğümüne snap'lenirken (hizalanırken) açısal uyum ceza ağırlığı (ters düğüme kilitlemeyi önler).
# • KONUM_FILTRE_AKTIF: GPS konum ve yaw verilerindeki ani sıçramalar ve gürültüler için filtrelemeyi aktif eder.
# • KONUM_FILTRE_ALPHA: Düşük geçişli (EMA) filtresinin hassasiyet çarpanıdır (yeni veriye yüzde kaç oranla güvenileceği, 0.0 - 1.0).
# • KONUM_JUMP_LIMIT_MS: İki konum okuması arasında izin verilen maksimum araç hızıdır (bu hızın üstündeki sıçramalar gürültü / outlier kabul edilerek elenir, m/s).
# ─────────────────────────────────────────────────────────────────────────────
...
ILERI_MESAFE_M      = 1.0    # start + sapma "burun" noktası: aracın yaw yönünde bu kadar ileri (m)
...
ARAC_MAX_DIREKSIYON    = math.radians(28.95) # Bee1 bisiklet eşdeğeri (ackermann.py max_bicycle_angle)
...
# ── TEST PARAMETRELERİ ───────────────────────────────────────────────────────
TEST_YOLU_KILIT_5S = True  # True -> Gerekli verileri (ilk konumu ve rotayı) aldıktan 5 sn sonra yolu kilitle, hiç değiştirme
...
#   Yukarıdaki sabitleri ÇALIŞIRKEN override eder (~1 sn).
...
try:
    ...
    sys.stderr.write(f"[hedef] canli_params yok, statik parametreler: {_canli_e}\n")
