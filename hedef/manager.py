...
from config import (
    ...
    SECILEN_SENARYO, CEZA_KARSIN_GECIS, CEZA_KARSIN_SEYIR, TEST_YOLU_KILIT_5S
)
...
class HedefYoneticisi:
    def __init__(self):
        ...
        self._ilk_rota_hesaplama_zamani = None  # TEST: ilk başarılı rota hesaplama zamanı
    ...
    def recalculate_path_from_robot(self, reason: str = "?",
        ...
        # ── TEST: 5 SN SONRA YOL KİLİTLEME ──
        if TEST_YOLU_KILIT_5S and self._ilk_rota_hesaplama_zamani is not None:
            if time.time() - self._ilk_rota_hesaplama_zamani > 5.0:
                rospy.loginfo_throttle(
                    5.0, f"[TEST] 5 saniye gecti, olusturulan yol kilitli. Reroute atlandi ({reason})."
                )
                return
        ...
        # ── VİRAJ İÇİ SAPMA KİLİDİ ──
        # Eğer önümüzdeki 10 waypoint içerisinde 20 dereceden fazla dönüş varsa (yani virajdaysak)
        # ve tetiklenme nedeni sadece "sapma" ise, rotayı bozmamak için yeniden planlamayı engelle.
        if reason == "sapma" and self.full_path_world:
            with self._wp_lock:
                lookahead = self.full_path_world[self.current_wp_index : self.current_wp_index + 10]
            if len(lookahead) >= 2:
                max_deg, _ = rota_donus_metrigi(lookahead, self.robot_yaw if self.robot_yaw is not None else 0.0)
                if max_deg > 20.0:
                    rospy.loginfo_throttle(
                        2.0, f"[kilit] Viraj içi kilit aktif (viraj açısı: {max_deg:.1f}°) → "
                             f"sapma recalc engellendi.")
                    return
        ...
        def snap_cost(n):
            if self._son_sag_serit and n in self._karsi_poz:
                return float('inf')
        ...
        if path and not rota_reddedildi:
            with self._wp_lock:
                ...
                if TEST_YOLU_KILIT_5S and self._ilk_rota_hesaplama_zamani is None:
                    self._ilk_rota_hesaplama_zamani = time.time()
                    rospy.loginfo(f"[TEST] İlk rota hesaplandi, 5 sn kilit sayaci basladi: {self._ilk_rota_hesaplama_zamani}")
