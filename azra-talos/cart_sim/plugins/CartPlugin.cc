...
CartPlugin::CartPlugin()
    : dataPtr(new CartPluginPrivate)
{
  int argc = 0;
  char *argv = nullptr;
  ros::init(argc, &argv, "CartPluginPrivate");
  this->robot_namespace_ = "";
  this->dataPtr->directionState = CartPluginPrivate::FORWARD;
...
void CartPlugin::OnCartCommand(const cart_sim::cart_control::ConstPtr &msg)
{
  ...
  {
    case cart_sim::cart_control::NEUTRAL:
      this->dataPtr->directionState = CartPluginPrivate::NEUTRAL;
      break;
    case cart_sim::cart_control::FORWARD:
      this->dataPtr->directionState = CartPluginPrivate::FORWARD;
      break;
    case cart_sim::cart_control::REVERSE:
      this->dataPtr->directionState = CartPluginPrivate::REVERSE;
      break;
    default:
      break;
  }
...
/////////////////////////////////////////////////
void CartPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  gzwarn << "CartPlugin loading params" << std::endl;
  // shortcut to this->dataPtr
  CartPluginPrivate *dPtr = this->dataPtr.get();
 ...
 //this->dataPtr->controlSub = nh.subscribe("prius", 10, &CartPlugin::OnPriusCommand, this);
  this->dataPtr->controlSub = nh.subscribe("cart", 10, &CartPlugin::OnCartCommand, this);

  this->dataPtr->node.Subscribe("/prius/reset",
      &CartPlugin::OnReset, this);
  this->dataPtr->node.Subscribe("/prius/stop",
      &CartPlugin::OnStop, this);

  this->dataPtr->node.Subscribe("/cmd_vel", &CartPlugin::OnCmdVel, this);
  this->dataPtr->node.Subscribe("/cmd_gear",
      &CartPlugin::OnCmdGear, this);
  this->dataPtr->node.Subscribe("/cmd_mode",
      &CartPlugin::OnCmdMode, this);
  ...
  this->dataPtr->updateConnection = event::Events::ConnectWorldUpdateBegin(
      std::bind(&CartPlugin::Update, this));

  this->dataPtr->keyboardSub =
    this->dataPtr->gznode->Subscribe("~/keyboard/keypress",
        &CartPlugin::OnKeyPress, this, true);

  this->dataPtr->worldControlPub =
    this->dataPtr->gznode->Advertise<msgs::WorldControl>("~/world_control");

  this->dataPtr->node.Subscribe("/keypress", &CartPlugin::OnKeyPressIgn,
      this);
}

/////////////////////////////////////////////////
void CartPlugin::OnCmdVel(const ignition::msgs::Pose &_msg)
{
...
/////////////////////////////////////////////////
void CartPlugin::OnCmdGear(const ignition::msgs::Int32 &_msg)
{
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
  ...
  this->dataPtr->directionState =
      static_cast<CartPluginPrivate::DirectionType>(state);
}

/////////////////////////////////////////////////
void CartPlugin::OnCmdMode(const ignition::msgs::Boolean &/*_msg*/)
{
  // toggle ev mode
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
...
/////////////////////////////////////////////////
void CartPlugin::KeyControlTypeA(const int _key)
{
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);

  switch (_key)
  {
    // w - gas pedal (hızlanma)
    case 87:
    case 119:
    {
    ...
    // d - steer right (direksiyonu sağa)
    case 68:
    case 100:
    {
      this->dataPtr->handWheelCmd -= 0.25;
    ...
    // r - release all (tüm kontrolleri serbest bırak)
    case 82:
    case 114:
    {
      this->dataPtr->brakePedalPercent = 0.0;
...
/////////////////////////////////////////////////
void CartPlugin::KeyControlTypeB(const int _key)
{
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
  ...
  switch (_key)
  {
    ...
    {
      this->dataPtr->brakePedalPercent = 0.0;
      if (this->dataPtr->directionState != CartPluginPrivate::REVERSE)
        this->dataPtr->gasPedalPercent = 0.0;
      this->dataPtr->gasPedalPercent += 0.1;
      this->dataPtr->gasPedalPercent =
          std::min(this->dataPtr->gasPedalPercent, 1.0);
...
/////////////////////////////////////////////////
void CartPlugin::KeyControl(const int _key)
{
  gzmsg << "KeyControl received key: " << _key << " (char: " << (char)_key << ")" << std::endl;
...
/////////////////////////////////////////////////
void CartPlugin::OnKeyPress(ConstAnyPtr &_msg)
{
  this->KeyControl(_msg->int_value());
}

/////////////////////////////////////////////////
void CartPlugin::OnKeyPressIgn(const ignition::msgs::Any &_msg)
{
  this->KeyControl(_msg.int_value());
}

/////////////////////////////////////////////////
void CartPlugin::OnReset(const ignition::msgs::Any & /*_msg*/)
{
...
/////////////////////////////////////////////////
void CartPlugin::OnStop(const ignition::msgs::Any & /*_msg*/)
{
...
void CartPlugin::Update()
{
  ...
  bool neutral = dPtr->directionState == CartPluginPrivate::NEUTRAL;
  ...
  //this->dataPtr->handWheelJoint->SetPosition(0, this->dataPtr->frWheelSteeringJoint->Position(0)/this->dataPtr->steeringRatio);
...
//  this->dataPtr->handWheelJoint->SetHighStop(0, this->dataPtr->handWheelCmd);
  ...
  if ((curTime - this->dataPtr->lastMsgTime) > .5)
  {
    ...
    if (this->dataPtr->directionState == CartPluginPrivate::FORWARD)
      consoleMsg.add_data(1.0);
    else if (this->dataPtr->directionState == CartPluginPrivate::REVERSE)
      consoleMsg.add_data(2.0);
    else if (this->dataPtr->directionState == CartPluginPrivate::NEUTRAL)
...
/////////////////////////////////////////////////
void CartPlugin::UpdateHandWheelRatio()
{
  // The total range the steering wheel can rotate
  this->dataPtr->handWheelHigh = 3.14;
  this->dataPtr->handWheelLow = -3.14;
...
double CartPlugin::CollisionRadius(physics::CollisionPtr _coll)
{
...
/////////////////////////////////////////////////
double CartPlugin::GasTorqueMultiplier()
{
