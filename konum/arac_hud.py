...
class RosBridge(QObject):
    ...
    def __init__(self):
        ...
        self._received = False   # FIX: ilk mesaj gelmeden bağlantı-kaybı sinyali verme
        self.timer = QTimer()
        self.timer.timeout.connect(self.check_connection)
        self.timer.start(100) # 100ms'de bir kontrol et

    def start_subscriber(self):
        rospy.Subscriber('/konum', Pose2D, self.odom_callback, queue_size=1)
    ...
    def check_connection(self):
        # FIX: ilk mesaj gelmeden last_msg_time=0.0 ile yanlış "bağlantı kaybı"
        # sinyali verilmesin (use_sim_time'da rospy.get_time() açılışta >1.0 olur).
...
class HudWindow(QWidget):
    ...
    def update_data(self, x, y, yaw):
        # FIX(perf): panel stylesheet'i her /konum mesajında uygulamak tam style
        # recalc + repaint tetikliyordu (50Hz'de gereksiz yük). Sadece bağlantı
        # yeniden kurulduğu geçişte uygula.
    ...
    def handle_signal_loss(self):
        # Sadece bağlıyken kopuşa geçişte stylesheet uygula (tekrar tekrar değil)
