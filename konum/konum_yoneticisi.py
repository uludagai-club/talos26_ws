...
class KonumYoneticisi:
    def __init__(self):
        ...
        # 3. HEDEF -> Samed
        rospy.Subscriber('/hedef', String, self.hedef_callback, queue_size=5)
    ...
    def hedef_callback(self, msg):
        ...
        # FIX(kalite): çıplak except → tipli; KeyboardInterrupt/SystemExit yutulmasın.
        try:
            ...
            # Yeni formatı dene: wx1,wy1|wx2,wy2 (|2'den fazla olabilir → maxsplit=1)
            try:
                if '|' in msg.data:
                    wp1_str = msg.data.split('|', 1)[0]
                    parts = wp1_str.split(',')
                ...
                rospy.logwarn_throttle(5.0, f"[konum] hedef parse hatası: {msg.data!r} ({e})")

    def publish_pose(self):
        """ Konum verilerini birlestir (Odom → x,y; IMU → yaw) ve /konum'a yayinla.
        NOT: Bu bir FÜZYON (EKF/UKF) DEĞİL — iki kaynaktan basit birleştirme. """
        pose = Pose2D()
